...
generic() {
  ...
  if [ ! -d $grubenv_repo_vc_dir ]; then
    git clone -b add-initramfs-image https://github.com/mirzak/grub-mender-grubenv.git $grubenv_dir >> "$build_log" 2>&1
  fi
  ...
  sudo sed -i 's/\"RootfsPartA\":.*/\"RootfsPartA\": '\"\\/dev\\/mapper\\/rootfsa\",'/g' ${target_rootfs_dir}/etc/mender/mender.conf
  sudo sed -i 's/\"RootfsPartB\":.*/\"RootfsPartB\": '\"\\/dev\\/mapper\\/rootfsb\",'/g' ${target_rootfs_dir}/etc/mender/mender.conf
...
EOF
  ...
  cat <<- 'EOF' | sudo tee ${target_rootfs_dir}/etc/fstab 2>&1 >/dev/null
# Autogenerated by the mender-convert tool

/dev/root    xfs    defaults    0 0
UUID=216f62f2-4abe-4efe-bdbe-c31f821803e8 /boot    xfs    defaults    0 0
/dev/mapper/mender-swap   swap    swap    defaults    0 0
...
# CentOS hacks. END
  ...
  #make >> "$build_log" 2>&1
  #log "\tInstalling GRUB2 boot scripts and tools."
  #sudo make DESTDIR=${target_boot_dir} install-boot-files >> "$build_log" 2>&1
  #sudo make DESTDIR=${target_rootfs_dir} install-tools >> "$build_log" 2>&1
}
